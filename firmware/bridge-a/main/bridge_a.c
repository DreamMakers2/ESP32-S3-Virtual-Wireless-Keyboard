...
typedef struct { uint8_t bytes[128]; size_t length; } cdc_chunk_t;
...
static void cdc_write_packet(const bridge_packet_t *packet) {
    ...
    framed[framed_len++]=0;
    if (xSemaphoreTake(cdc_write_lock,pdMS_TO_TICKS(20)) == pdTRUE) { tinyusb_cdcacm_write_queue(TINYUSB_CDC_ACM_0,framed,framed_len); tinyusb_cdcacm_write_flush(TINYUSB_CDC_ACM_0,0); xSemaphoreGive(cdc_write_lock); }
}
...
static void cdc_rx_callback(int itf, cdcacm_event_t *event) {
    (void)event; cdc_chunk_t chunk={0}; size_t read=0; if (tinyusb_cdcacm_read(itf,chunk.bytes,sizeof(chunk.bytes),&read)==ESP_OK && read) { chunk.length=read; (void)xQueueSend(cdc_rx_queue,&chunk,0); }
}
...
static void handle_cdc_packet(const bridge_packet_t *p) {
    uint32_t now=now_ms();
    lock_state();
...
static void cdc_task(void *ignored) {
    (void)ignored; cdc_chunk_t chunk; uint8_t frame[BRIDGE_MAX_COBS]; size_t used=0; bool discarding=false;
    for (;;) if (xQueueReceive(cdc_rx_queue,&chunk,pdMS_TO_TICKS(10))==pdTRUE) for (size_t i=0;i<chunk.length;++i) {
        ...
        if (!c) {
            if (discarding) {
                ...
                if (n && bridge_packet_decode(raw,n,&p)) handle_cdc_packet(&p);
...
static void control_task(void *ignored) {
    (void)ignored; for (;;) { uint32_t now=now_ms();
        ...
        bool paused=peer_ready && !active && app_hello_seen && now-last_status_ms<=HOST_HELLO_TIMEOUT_MS && now-last_app_hello_ms<=HOST_HELLO_TIMEOUT_MS;
        bool ready=peer_ready;
        unlock_state();
...
void app_main(void) {
    ...
    tinyusb_config_t usb=TINYUSB_DEFAULT_CONFIG(); ESP_ERROR_CHECK(tinyusb_driver_install(&usb)); tinyusb_config_cdcacm_t acm={.cdc_port=TINYUSB_CDC_ACM_0,.callback_rx=cdc_rx_callback}; ESP_ERROR_CHECK(tinyusb_cdcacm_init(&acm));

...
The tested versions are in [requirements](../docs/REQUIREMENTS.md).
...
The launcher never installs packages. The packaged executable lives in `app/bin`;
...
Reconnection never activates capture automatically.
...
The nonselectable local history illustrates physical input; it is not synchronized
with the remote editor. Its top/bottom fade and scrollbar affect only that history.
Source interpretation uses the XKB layout selected by the environment; ensure it
matches the target's layout. The target receives physical HID usages.
...
includes the target-facing HID transfer; it does not measure target application
rendering or processing. No keystroke log is written to disk.
